...
const ToolsScreen: React.FC = () => {
  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 dark:bg-gray-900 pb-24">
      ...
      {/* Tab Switcher */}
      <div className="bg-white dark:bg-gray-800 p-4 pb-0 border-b border-gray-200 dark:border-gray-700 sticky top-0 z-20">
        <div className="flex bg-gray-100 dark:bg-gray-700 p-1 rounded-xl overflow-x-auto no-scrollbar">
          <button
            onClick={() => setActiveTab('tasbih')}
            className={`flex-1 min-w-[70px] flex items-center justify-center gap-2 py-2.5 text-xs sm:text-sm font-bold rounded-lg transition-all ${
              activeTab === 'tasbih' 
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {/* TASBIH VIEW */}
        {activeTab === 'tasbih' && (
          <div className="flex flex-col items-center justify-start min-h-[70vh] p-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
             
             {/* Selected Amol Header */}
             <div className="text-center mb-6 w-full">
                {selectedDhikr ? (
                   <div className="animate-in slide-in-from-top-2">
                       <h2 className="text-2xl font-bold font-arabic text-emerald-700 dark:text-emerald-400 mb-1">{selectedDhikr.arabic}</h2>
                       <p className="text-lg font-bold text-gray-800 dark:text-white">{language === 'bn' ? selectedDhikr.bangla : selectedDhikr.english}</p>
                       <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 uppercase tracking-widest">Target: {selectedDhikr.target}</p>
                   </div>
                ) : (
                    <>
                        <h1 className="text-xl font-bold text-gray-800 dark:text-white">{strings.tasbihTitle}</h1>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{strings.tasbihSubtitle}</p>
                    </>
                )}
             </div>

             {/* Counter Circle */}
             <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-2xl p-8 w-full max-w-xs aspect-[3/4] flex flex-col items-center justify-between border border-gray-100 dark:border-gray-700 relative transition-colors overflow-hidden mb-6">
               <div className="w-full bg-emerald-900 dark:bg-black rounded-xl p-6 mb-4 shadow-inner text-right font-mono border border-emerald-800 dark:border-gray-800 relative">
                  <span className={`text-5xl drop-shadow-md font-digital ${selectedDhikr && count >= selectedDhikr.target ? 'text-green-400 animate-pulse' : 'text-emerald-100'}`}>
                    {count.toString().padStart(selectedDhikr ? selectedDhikr.target.toString().length : 4, '0')}
                  </span>
                  {selectedDhikr && (
                      <span className="absolute bottom-2 left-3 text-xs text-emerald-400/50 font-bold">/ {selectedDhikr.target}</span>
                  )}
               </div>
               ...
               <button 
                 onClick={increment}
                 className={`w-40 h-40 rounded-full shadow-lg active:shadow-inner active:scale-95 transition-all flex items-center justify-center border-4 z-0 ${
               ...
               >
                 {selectedDhikr && count >= selectedDhikr.target ? (
                     ...
                     <div className="text-white/20 text-6xl select-none pointer-events-none">+</div>
                 )}
               </button>
        ...
        {/* TRACKER VIEW */}
        {activeTab === 'tracker' && (
          <div className="p-4 pt-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">{strings.trackerTitle}</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{strings.trackerSubtitle}</p>
            ...
            <div className="space-y-3 pb-8">
              {taskList.map((task) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <h3 className={`font-medium ${isCompleted ? 'text-emerald-900 dark:text-emerald-400' : 'text-gray-800 dark:text-gray-200'}`}>{task.label}</h3>
                    ...
                    <div className={isCompleted ? 'text-emerald-500 dark:text-emerald-400' : 'text-gray-300 dark:text-gray-600'}>
                      {isCompleted ? <CheckCircle2 size={24} fill="currentColor" className="text-white dark:text-emerald-950" /> : <Circle size={24} />}
                    </div>
                  </div>
                );
              })}
        ...
        {activeTab === 'quran' && (
          <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-300 relative">
             <div className="p-4 pb-2 sticky top-0 bg-gray-50 dark:bg-gray-900 z-10">
                <div className="flex items-center justify-between mb-4">
                  <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{strings.quranTitle}</h1>
                  <select 
                       value={selectedReciter} 
                       onChange={(e) => setSelectedReciter(parseInt(e.target.value))}
                       className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-xs rounded-lg p-1.5 outline-none focus:ring-2 focus:ring-emerald-500 max-w-[140px]"
                     >
                       {RECITERS.map(r => (
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-emerald-500 dark:text-white"
                  />
                     ...
                     return (
                       ...
                       >
                         <div className="flex items-center gap-4">
                           ...
                           <div>
                             <h3 className={`font-bold text-sm ${isSelected ? 'text-emerald-700 dark:text-emerald-400' : 'text-gray-800 dark:text-gray-200'}`}>
                               {surah.name_simple} 
                             </h3>
                             <p className="text-[10px] text-gray-500 dark:text-gray-400">{strings.verses}: {surah.verses_count}</p>
                           </div>
                         </div>
                         <div className="flex items-center gap-3">
                           <p className="font-arabic text-lg text-gray-800 dark:text-gray-300">{surah.name_arabic}</p>
                           {isPlayingThis && (
                             <div className="flex items-center gap-0.5">
                               <span className="w-1 h-3 bg-emerald-500 rounded-full animate-[pulse_1s_ease-in-out_infinite]"></span>
                               <span className="w-1 h-4 bg-emerald-500 rounded-full animate-[pulse_1.5s_ease-in-out_infinite]"></span>
                             </div>
                           )}
                         </div>
                       </button>
             ...
             {currentSurah && (
               <div className="absolute bottom-4 left-4 right-4 bg-gray-900 dark:bg-emerald-900 text-white p-4 rounded-2xl shadow-xl border border-white/10 flex items-center justify-between animate-in slide-in-from-bottom-2 z-30">
                 <div className="flex items-center gap-3 overflow-hidden">
                    <div className="p-2 bg-white/10 rounded-full"><Music size={20} /></div>
                    <div className="min-w-0">
                       <p className="text-xs text-gray-300 uppercase font-bold tracking-wider">{strings.nowPlaying}</p>
                       <p className="font-bold truncate text-sm">{currentSurah.name_simple}</p>
                    </div>
                 </div>
                 <button onClick={toggleQuranPlay} className="w-10 h-10 bg-emerald-500 rounded-full flex items-center justify-center text-white hover:bg-emerald-400 shadow-lg">
                    {isQuranPlaying ? <Pause size={20} fill="currentColor" /> : <Play size={20} fill="currentColor" className="ml-0.5" />}
                 </button>
               </div>
             )}
          </div>
        ...
        {activeTab === 'zakat' && (
          <div className="p-4 pt-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
             <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                   <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{strings.zakatTitle}</h1>
                   <p className="text-xs text-gray-500 dark:text-gray-400">{strings.zakatSubtitle}</p>
                </div>
             </div>
             <div className="space-y-6">
                <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                   ...
                   <div className="space-y-4">
                      <div>
                        <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1 ml-1">{strings.cash}</label>
                        <input 
                          type="number" 
                          placeholder="0" 
                          value={zakatData.cash || ''}
                          onChange={(e) => handleZakatChange('cash', e.target.value)}
                          className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-2.5 text-gray-800 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      <div>
                        <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1 ml-1">{strings.goldSilver}</label>
                        <input 
                          type="number" 
                          placeholder="0" 
                      ...
                      <div>
                        <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1 ml-1">{strings.businessAssets}</label>
                        <input 
                          type="number" 
                          placeholder="0" 
                ...
                <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
                   ...
                   <div>
                        <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1 ml-1">{strings.debts}</label>
                        <input 
                          type="number" 
                          placeholder="0" 
                          value={zakatData.debt || ''}
                          onChange={(e) => handleZakatChange('debt', e.target.value)}
                          className="w-full bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl px-4 py-2.5 text-gray-800 dark:text-white outline-none focus:ring-2 focus:ring-red-500"
                        />
                   </div>
                </div>

                {/* Result Section (Condensed for brevity as requested) */}
                <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-6 text-white shadow-xl">
                   <div className="flex justify-between items-center mb-2">
                     <span className="text-emerald-100 text-sm">{strings.totalAssets}</span>
                     <span className="font-bold font-mono">{strings.currency} {netAssets.toLocaleString()}</span>
                   </div>
                   <div className="h-px bg-white/20 my-2"></div>
                   <div className="flex justify-between items-center mt-2">
                     <span className="text-white font-bold">{strings.payableZakat}</span>
                     <span className="text-2xl font-bold font-mono">{strings.currency} {zakatPayable.toLocaleString()}</span>
                   </div>
                </div>
