...
const ProfileScreen: React.FC<{ onBack: () => void; onAdmin?: () => void }> = ({ onBack, onAdmin }) => {
  ...
  const fetchIpInfo = async () => {
    setShowIpInfo(true);
    if (ipData) return; // Prevent re-fetch if data exists
    
    setLoadingIp(true);
    setIpError(false);
    try {
      // Using ipwho.is as it's more reliable for client-side requests (CORS friendly)
      ...
      setIpData({
        ip: data.ip,
        // Map ipwho.is fields to match what the UI expects
    ...
    <div className="fixed inset-0 z-50 bg-white dark:bg-gray-950 flex flex-col animate-in slide-in-from-right duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          >
            <div className={`w-24 h-24 rounded-full flex items-center justify-center border-4 border-white dark:border-gray-800 shadow-md overflow-hidden bg-emerald-100 dark:bg-emerald-900 ${isUploading ? 'opacity-50' : ''}`}>
               {user?.photoURL ? (
            ...
            {!user?.isAnonymous && (
              <div className="absolute bottom-0 right-0 bg-emerald-600 p-2 rounded-full border-2 border-white dark:border-gray-900 shadow-sm z-10">
            ...
            {isEditing ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="bg-gray-100 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-1 text-center font-bold text-gray-800 dark:text-white outline-none focus:ring-2 focus:ring-emerald-500 w-40"
                  autoFocus
                />
                <button 
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">{user?.email || (user?.isAnonymous ? 'Anonymous' : '')}</p>
        ...
        {isAdmin && (
          <button 
            onClick={onAdmin}
            className="w-full mb-6 flex items-center justify-between p-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl shadow-lg hover:shadow-xl transition-all transform active:scale-95 group"
          >
             <div className="flex items-center gap-3">
               <ShieldCheck size={24} className="text-emerald-400 dark:text-emerald-600" />
               <div className="text-left">
                  <h3 className="font-bold text-lg">{strings.adminControl}</h3>
                  <p className="text-xs text-gray-400 dark:text-gray-500">Manage App Settings</p>
               </div>
             </div>
             <div className="bg-gray-800 dark:bg-gray-100 p-2 rounded-full group-hover:bg-gray-700 dark:group-hover:bg-gray-200 transition-colors">
        ...
        {/* Settings Section */}
        <div className="bg-gray-50 dark:bg-gray-900 rounded-2xl p-4 space-y-4 mb-6">
           {/* Language */}
           <div className="flex items-center justify-between">
             <div className="flex items-center gap-3">
               <Globe className="text-gray-400" size={20} />
               <span className="text-gray-700 dark:text-gray-200">{strings.language}</span>
             </div>
             <div className="flex bg-white dark:bg-gray-800 rounded-lg p-1 shadow-sm border border-gray-100 dark:border-gray-700">
               <button 
                 onClick={() => setLanguage('bn')}
                 className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${language === 'bn' ? 'bg-emerald-100 text-emerald-700' : 'text-gray-400'}`}
               >
                 বাংলা
               </button>
               <button 
                 onClick={() => setLanguage('en')}
                 className={`px-3 py-1 rounded-md text-xs font-medium transition-colors ${language === 'en' ? 'bg-emerald-100 text-emerald-700' : 'text-gray-400'}`}
               >
                 EN
               </button>
             </div>
           ...
           <div className="flex items-center justify-between cursor-pointer" onClick={fetchIpInfo}>
             <div className="flex items-center gap-3">
               ...
               <span className="text-gray-700 dark:text-gray-200">{strings.ipInfo}</span>
             </div>
             <div className="p-2 bg-white dark:bg-gray-800 rounded-full shadow-sm border border-gray-100 dark:border-gray-700 text-gray-400 dark:text-gray-300">
               <Wifi size={18} />
             </div>
           </div>
        </div>
        ...
        {/* About Section */}
        <div className="mt-8 mb-4 animate-in fade-in slide-in-from-bottom-2 duration-500 delay-100">
            ...
            <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm">
                ...
                {appConfig.donation.enabled && (
                  <div 
                     ...
                     className="p-4 flex items-center justify-between border-b border-gray-50 dark:border-gray-800 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
                  >
                      <div className="flex items-center gap-3">
                          <Heart size={18} className="text-rose-500 fill-current" />
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-200">{strings.donate}</span>
                      </div>
                      <span className="text-[10px] font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/20 px-2 py-1 rounded-full border border-rose-100 dark:border-rose-900/30">
                          Support Us
                      </span>
                  </div>
                )}
                ...
                >
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-200">Feedback & Support</span>
                    </div>
                    <span className="text-[10px] font-bold text-blue-500 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded-full border border-blue-100 dark:border-blue-900/30">
                        Contact Admin
                    </span>
                </div>

                <div className="p-4 flex items-center justify-between border-b border-gray-50 dark:border-gray-800">
                    <span className="text-sm text-gray-600 dark:text-gray-300">{strings.version}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-white bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded text-xs font-mono">v{appConfig.version}</span>
                </div>
                <div 
                ...
                >
                    <span className="text-sm text-gray-600 dark:text-gray-300">{strings.developer}</span>
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-emerald-600 dark:text-emerald-400">{appConfig.developer.name}</span>
        ...
        <button 
          onClick={logout}
          className="w-full flex items-center justify-center gap-2 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 py-3.5 rounded-xl font-bold hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors mt-8"
        >
          <LogOut size={18} />
          {strings.signOut}
        </button>
