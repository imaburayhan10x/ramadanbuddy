      ...
      <div className="mb-4 space-y-4 flex-shrink-0">
        ...
        <div className="relative">
          <input
            ...
            className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-3 pl-10 pr-4 outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm dark:text-white transition-all"
          />
          <Search className="absolute left-3 top-3.5 text-gray-400" size={18} />
          <button 
            onClick={handleSearchClick}
            disabled={isSearching || !query.trim()}
            className="absolute right-2 top-2 bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 px-3 py-1 rounded-lg text-xs font-bold"
          >
      ...
      <div className="space-y-4 flex-1">
        ...
        {!isSearching && hasSearched && searchResults.length === 0 ? (
          ...
          !isSearching && displayList.map((dua, index) => (
            <div key={dua.id || index} className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm border border-gray-100 dark:border-gray-700 transition-colors animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  <h2 className="font-bold text-emerald-800 dark:text-emerald-400 text-lg">
                    {language === 'bn' ? dua.title : (dua.titleEn || dua.title)}
                  </h2>
                  {(dua.source || dua.reference) && (
                    <div className="flex flex-wrap items-center gap-2 mt-2">
                      <span className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 text-[10px] px-2 py-1 rounded-md border border-emerald-100 dark:border-emerald-800 flex items-center gap-1 font-medium">
                         <Database size={10} /> {dua.source || strings.verifiedSource}
                      </span>
                      {dua.reference && (
                        <span className="text-[10px] text-gray-500 dark:text-gray-400 font-medium bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded-md border border-gray-200 dark:border-gray-600">
              ...
              <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg mb-4 border-r-4 border-emerald-500/30">
                <p className="text-right text-xl leading-10 font-arabic text-gray-800 dark:text-gray-100 font-medium">
                  {dua.arabic}
                </p>
              </div>

              <div className="space-y-3">
                <p className="text-sm text-gray-600 dark:text-gray-300 italic pl-3 border-l-2 border-gray-200 dark:border-gray-700">
                  <span className="font-medium text-emerald-700 dark:text-emerald-500 block text-xs mb-1 uppercase tracking-wider opacity-70">{strings.pronunciation}</span> 
                  {language === 'bn' ? dua.bangla : (dua.english || dua.bangla)}
                </p>
                <div className="pt-2">
                   <span className="font-medium text-gray-900 dark:text-gray-200 block text-xs mb-1 uppercase tracking-wider opacity-70">{strings.meaning}</span>
