...
const HomeScreen: React.FC<HomeScreenProps> = ({ onOpenProfile, onNavigate }) => {
  ...
  return (
    <div className="pb-24 pt-4 px-4 space-y-5 relative min-h-screen overflow-x-hidden">
      ...
      <div className="relative z-10 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border border-slate-100/50 dark:border-slate-800 shadow-sm rounded-2xl p-4 flex justify-between items-center mb-6">
        <div 
          onClick={() => setShowLocationModal(true)}
          className="cursor-pointer active:opacity-70 transition-opacity flex items-center gap-3"
        >
          {appConfig.appLogo ? (
            <img src={appConfig.appLogo} alt="Logo" className="w-10 h-10 object-contain drop-shadow-sm" />
          ) : (
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white drop-shadow-sm leading-none">{appConfig.appName}</h1>
        ...
        <div className="relative z-10 p-6 text-white">
          <div className="flex justify-between items-start mb-6 pb-4 border-b border-white/10">
            <div>
               ...
               <div className="text-3xl font-bold font-digital tracking-wide drop-shadow-sm">
                 {displayTime || "--:-- --"}
               </div>
               {timings && <div className="text-[10px] text-emerald-200 mt-1">{timings.timezone}</div>}
            </div>
            <div className="text-right">
              <div className="text-[10px] uppercase tracking-wider font-bold text-emerald-200 mb-1">
                {nextEventLabel} Remaining
              </div>
              <div className="flex items-center justify-end gap-1.5 bg-black/20 px-2 py-1 rounded-lg backdrop-blur-md border border-white/10">
                 <Timer size={14} className="text-emerald-300" />
                 <span className="font-mono font-bold text-lg">{countdown || "00h 00m 00s"}</span>
              </div>
            </div>
          </div>
          ...
          {timings ? (
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-black/20 dark:bg-slate-800/50 rounded-2xl p-4 border border-white/10 backdrop-blur-sm hover:bg-black/30 transition-colors">
                <div className="flex items-center gap-2 text-emerald-200 mb-2">
                  <Moon size={16} />
                  <span className="text-xs font-bold uppercase tracking-wider">{strings.sehriEnds}</span>
                </div>
                <p className="text-3xl font-bold">{timings.sehri}</p>
              </div>

              <div className="bg-white/10 dark:bg-slate-700/50 rounded-2xl p-4 border border-white/20 backdrop-blur-sm hover:bg-white/15 transition-colors">
                <div className="flex items-center gap-2 text-amber-200 mb-2">
                  <Sun size={16} />
                  <span className="text-xs font-bold uppercase tracking-wider">{strings.iftarStarts}</span>
                </div>
                <p className="text-3xl font-bold">{timings.iftar}</p>
              </div>
            </div>
          ) : (
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800 relative overflow-hidden group transition-all">
         ...
         <div className="flex items-center justify-between mb-3 relative z-10">
            <h3 className="font-bold text-slate-700 dark:text-slate-200 text-sm flex items-center gap-2">
               <Sparkles size={14} className="text-amber-500" /> Asma-ul-Husna
            </h3>
         ...
         <div className="relative">
             ...
             <button 
                ...
                className="absolute left-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-slate-400 hover:text-emerald-600 transition-all shadow-sm -ml-2"
             >
                <ChevronLeft size={20} />
             </button>
             ...
             <button 
                ...
                className="absolute right-0 top-1/2 -translate-y-1/2 z-20 p-2 rounded-full bg-slate-50 dark:bg-slate-800 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-slate-400 hover:text-emerald-600 transition-all shadow-sm -mr-2"
             >
                <ChevronRight size={20} />
             </button>

             {/* Content */}
             <div className="text-center py-4 relative z-10 transition-all duration-500 px-8">
                <h2 className="text-4xl font-arabic font-bold text-emerald-600 dark:text-emerald-400 mb-2 drop-shadow-sm select-none">
                   {ALLAH_NAMES[nameIndex].arabic}
                </h2>
                <p className="text-lg font-bold text-slate-800 dark:text-white select-none">
      ...
      {/* AI Tip */}
      <div className="bg-indigo-50 dark:bg-slate-900 border border-indigo-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
        <h3 className="text-indigo-800 dark:text-indigo-400 font-bold text-sm mb-2 flex items-center gap-2">
          ✨ {strings.todaysTip}
        </h3>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-5 shadow-sm border border-slate-100 dark:border-slate-800">
        ...
        >
          <h3 className="text-slate-800 dark:text-slate-200 font-bold border-l-4 border-emerald-500 pl-3">
            {strings.todaysDua}
          </h3>
          <span className="text-xs text-emerald-600 dark:text-emerald-400 flex items-center gap-1 group-hover:translate-x-1 transition-transform font-bold">
            {strings.viewAll} <ChevronRight size={14} />
          </span>
        </div>
        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-700/50">
           <p className="text-center font-arabic text-2xl text-slate-800 dark:text-slate-200 leading-loose mb-3">
             {DUAS[0].arabic}
           </p>
